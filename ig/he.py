...
חוק השם של הדבר (הבעלים, 10.9): כל דבר בסיפור נקרא על שם מה שעושים איתו, לא על שם ההגדרה הרשמית שלו — גם כשהפוסט האנגלי עצמו כתב "מודל" או "תרחישים" או "צמיחה כלכלית". אם זה בעצם אתר שמקלידים בו את המקצוע ורואים מה יקרה לו עד 2030, זה בדיוק מה שכותבים. הפוסט האנגלי הוא דף עובדות, לא מילון: קח ממנו את המספרים והשמות, אף פעם לא את המילים. לפני שאתה סוגר שקופית, אמור אותה בקול לחבר ליד השולחן — משפט שלא היית אומר בקול, נכתב מחדש.
...
def localize(post_dir):
    ...
    # Carousel-video copying retired (owner order Sep 14, single-picture
    # posts): post.py publishes ONLY slide-1.jpg now, and write.py no
    # longer produces video-N.mp4 files, so there is nothing to carry over.
    import render_he
