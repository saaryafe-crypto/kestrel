...
/* cover: getintoai band anatomy, ported from render.py (owner Sep 14 —
   covers now generate 4:3 to match the landscape photo window; the old
   Aug 22 full-frame fade would side-crop a 4:3 image by ~40%. The photo
   owns the TOP at FULL visibility and melts into solid black right above
   the masthead; the title never prints over the picture.) */
...
def render(post_path, out_dir):
    ...
    for n, s in enumerate(slides, 1):
        ...
        # naked-slide gate, render half (owner audit Sep 10, same as
        # render.py): a content slide with no real image is named LOUD in the
        # CI log — the blur fallback below is camouflage, not media.
        if (s["type"] == "content" and not m
                and s.get("layout") not in ("break", "tweet")):
            print(f"WARNING: slide {n} renders NAKED — no real media, only "
                  "the blur fallback; the winners never ship this",
                  file=sys.stderr)
