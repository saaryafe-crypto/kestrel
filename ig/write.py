...
def simpler_brief(brief, headline, flaw="", mode="simpler"):
    ...
    if mode == "concept":
        ...
        task = ('This is a guide-lane cover: rewrite as ONE short plain '
                ...
                'famous names exactly as written. A logo next to a person '
                'is said exactly as "the OpenAI logo in a big circle '
                'next to him" (owner Sep 16-17).')
    else:
...
def art_direct(post, story_title="", simple=False):
    ...
    logo_note = ((
        ...
        'LOGO PLACEMENT (owner Sep 16, from his two reference covers): a '
        'logo is used only when the story needs it, not on every cover. '
        'When a person is in frame, the brief places it with these exact '
        'words and nothing more: "the OpenAI logo in a big circle next '
        'to him" (owner Sep 17: "big" — the small ones rendered too '
        'small). ANTHROPIC IS ALWAYS CLAUDE (owner Sep 17: Claude is '
        'more known = higher conversion): an Anthropic story uses "the '
        'Claude logo", NEVER "the Anthropic logo" — in the brief text '
        'AND in the returned "logo" field, always claude. Never on '
        'walls, screens, signs or products — never '
        '"logo sign", "backlit sign", "neon sign". The mark is the '
        "brand's SYMBOL, never its name spelled out. A brand NOT on the "
        "list gets no logo and no lettering at all — carry it through "
        "its product's real shape and colors instead. "
...
PRODUCT-HERO (owner's gold standard, Aug 1 — the @technology Codex Micro reference; MANDATORY for the COVER whenever the story is a famous company's physical product or gadget): the company's famous CEO (full name in the brief text AND in "face" — see the famous-people rule) HOLDS the product chest-high toward the camera with both hands, chest-up, eyes to camera, with the company's logo in a big circle next to them (owner Sep 16-17 placement law — never a wall sign). {ref_note} The person presents, the logo brands, the device IS the story — all three connected.
...
3. THE LOGOS — only when the story needs a mark (owner Sep 16, not every cover): "with the X logo in a big circle next to him" (his reference covers: one circle badge beside the person; "big" since Sep 17 — the small ones rendered too small), or "with relevant AI logos" when several belong (and return "logo" so the real mark rides on ref rungs).
...
- THE BRAND RIDES AS A REFERENCE: when a listed brand belongs in the picture, the plain line says so ("with the OpenAI logo in a big circle next to him", "with relevant AI logos") and you return "logo" so the real mark rides to the generator — never described staging ("glowing backlit sign"), never drawn from memory. The renderer stamps nothing on top: if the brand isn't in the scene it isn't on the cover.
        ...
        for b in r.get("briefs", []):
            ...
            if isinstance(i, int) and 0 <= i < len(post["slides"]) and b.get("brief", "").strip():
                ...
                if b.get("logo", "").strip():  # real vector mark = logo ref
                    lg = b["logo"].strip().lower()
                    # owner Sep 17: Claude > Anthropic (more known = higher
                    # conversion) — mechanical, prompt slips can't ship it
                    if lg == "anthropic":
                        lg = "claude"
                        post["slides"][i]["image_brief"] = re.sub(
                            r"(?i)\bAnthropic logo\b", "Claude logo",
                            post["slides"][i]["image_brief"])
                    post["slides"][i]["gen_logo"] = lg
                n += 1
