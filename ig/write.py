...
G) SCENE OF THE STORY (fame bar fails on the actor, no product to show) → FIRST re-check the cast: the famous face may come from EITHER side of the story, victim included — an anonymous-hacker story still has the attacked company's famous CEO (owner lesson Sep 8, the $320M Liquid heist: the winning cover was Blockstream's CEO caught inside the emptied vault, "wait, what just happened to Blockstream?" — not exploding coins). If ANY side has a famous face, cast that person AT the scene of the consequence via archetype B/C staging. Only when no side has one: SUBJECT is the story's real place staged at its most dramatic aftermath moment, shot like a leaked documentary press photo — the massive vault door swung wide open on an EMPTY vault, emergency lights on wet concrete, one glowing terminal casting light — cinematic chiaroscuro, deep blacks, the two palette colors as saturated glowing accents cutting the dark. SYMBOL CLICHE BAN (owner Sep 8, the coin-explosion post-mortem): floating or exploding coins, raining money, giant cracking tokens, glowing padlocks, abstract shockwaves — these read as 2017 crypto-scam stock renders and scrollers tune them out instantly. WHY: a real place mid-story makes the viewer ask "what happened HERE" and stop for the caption; an abstract symbol answers the question before it is asked.
...
- PALETTE by the story's emotion: threat/ban → red and deep blue; money → green and gold; win/launch → orange and white; crash/scandal → red and black. Two colors, no more. The palette tones the BACKDROP and accent props only — the subject always keeps its true-to-life colors so it pops (owner Sep 7: an all-orange cover read as one flat wash, "boring and cant understand").
- LABELS are legal on covers ONLY as briefed chips: 1-3 words each, max 3 chips, quoted exactly in the brief. DEFAULT IS ZERO CHIPS (owner Sep 8, the "$320M" receipt-chip post-mortem: forcing the model to render exact text produces garbled artifacts that destroy photorealism): brief chips ONLY where the archetype demands them — E's comparison sides. Everywhere else the scene makes the number physical and the headline system carries every word.
