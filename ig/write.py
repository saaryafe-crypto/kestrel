...
def simpler_brief(brief, headline, flaw="", mode="simpler"):
    ...
    if mode == "concept":
        ...
        task = ('This is a guide-lane cover: rewrite as ONE short plain '
                ...
                'is said exactly as "the OpenAI logo in a small circle '
                'next to him" (owner Sep 16).')
    else:
...
def art_direct(post, story_title="", simple=False):
    ...
    logo_note = ((
        ...
        'words and nothing more: "the OpenAI logo in a small circle next '
        'to him". Never on walls, screens, signs or products — never '
...
PRODUCT-HERO (owner's gold standard, Aug 1 — the @technology Codex Micro reference; MANDATORY for the COVER whenever the story is a famous company's physical product or gadget): the company's famous CEO (full name in the brief text AND in "face" — see the famous-people rule) HOLDS the product chest-high toward the camera with both hands, chest-up, eyes to camera, with the company's logo in a small circle next to them (owner Sep 16 placement law — never a wall sign). {ref_note} The person presents, the logo brands, the device IS the story — all three connected.
...
3. THE LOGOS — only when the story needs a mark (owner Sep 16, not every cover): "with the X logo in a small circle next to him" (his reference covers: one small circle badge beside the person), or "with relevant AI logos" when several belong (and return "logo" so the real mark rides on ref rungs).
...
- THE BRAND RIDES AS A REFERENCE: when a listed brand belongs in the picture, the plain line says so ("with the OpenAI logo in a small circle next to him", "with relevant AI logos") and you return "logo" so the real mark rides to the generator — never described staging ("glowing backlit sign"), never drawn from memory. The renderer stamps nothing on top: if the brand isn't in the scene it isn't on the cover.
